...
//Destructor/Constructors
    ...
    template< class T >
    HashGraph<T>::HashGraph() : node_values()
    {
    }
...
//Copy all nodes and edges from g
    template< class T >
    HashGraph<T>::HashGraph( const HashGraph& g ) :node_values( g.node_values ), edge_values( g.edge_values )
    {
    }
...
//  throw a GraphError exception with appropriate descriptive text
    ...
    {
        if ( !this->has_edge( origin, destination ))
        {
            std::ostringstream error;
            error << "HashGraph::edge_value: edge: " << origin << "->" << destination << "is not in the graph";
            throw ( ics::GraphError( error.str()));
        }
...
//  throw a GraphError exception with appropriate descriptive text
    ...
    int HashGraph<T>::in_degree( NodeName node_name ) const
    {
        ...
        {
            std::ostringstream error;
            error << "HashGraph::in_degree: node: " << node_name << " is not in the graph";
            throw ( ics::GraphError( error.str()));
        }
...
//  throw a GraphError exception with appropriate descriptive text
    ...
    int HashGraph<T>::out_degree( NodeName node_name ) const
    {
        ...
        {
            std::ostringstream error;
            error << "HashGraph::out_degree: node: " << node_name << " is not in the graph";
            throw ( ics::GraphError( error.str()));
        }
...
//  throw a GraphError exception with appropriate descriptive text.
    ...
    int HashGraph<T>::degree( NodeName node_name ) const
    {
        ...
        {
            std::ostringstream error;
            error << "HashGraph::degree: node: " << node_name << " is not in the graph";
            throw ( ics::GraphError( error.str()));
        }

        const auto& nodeInfo = node_values[ node_name ];
        return nodeInfo.in_edges.size() + nodeInfo.out_edges.size();
    }
...
//  appropriate  descriptive text
    ...
    auto HashGraph<T>::out_nodes( NodeName node_name ) const -> const NodeSet&
    {
        ...
        {
            std::ostringstream error;
            error << "HashGraph::out_nodes: node: " << node_name << " is not in the graph";
            throw ( ics::GraphError( error.str()));
        }
...
//  appropriate descriptive text
    ...
    auto HashGraph<T>::in_nodes( NodeName node_name ) const -> const NodeSet&
    {
        ...
        {
            std::ostringstream error;
            error << "HashGraph::in_nodes: node: " << node_name << " is not in the graph";
            throw ( ics::GraphError( error.str()));
        }
...
//  appropriate descriptive text
    ...
    auto HashGraph<T>::out_edges( NodeName node_name ) const -> const EdgeSet&
    {
        ...
        {
            std::ostringstream error;
            error << "HashGraph::out_edges: node: " << node_name << " is not in the graph";
            throw ( ics::GraphError( error.str()));
        }
...
//  appropriate descriptive text
    ...
    auto HashGraph<T>::in_edges( NodeName node_name ) const -> const EdgeSet&
    {
        ...
        {
            std::ostringstream error;
            error << "HashGraph::in_edges: node: " << node_name << " is not in the graph";
            throw ( ics::GraphError( error.str()));
        }
...
//Add these node names and update edge_values and the LocalInfos of each node
    ...
    {
        auto& originInfo = node_values[ origin ];
        originInfo.out_nodes.insert( destination );
        originInfo.out_edges.insert( Edge( origin, destination ));
        auto& destInfo = node_values[ destination ];
        destInfo.in_nodes.insert( origin );
        destInfo.in_edges.insert( Edge( origin, destination ));
...
// to copy a set and then iterate over it while removing values from the original set
    ...
    void HashGraph<T>::remove_node( NodeName node_name )
    {
        if ( node_values.has_key( node_name ))
        {
            ...
            for ( NodeMapEntry& mapEntry : node_values )
            {
                LocalInfo& entryInfo = mapEntry.second;

                entryInfo.out_nodes.erase( node_name );
                entryInfo.in_nodes.erase( node_name );

                EdgeSet outEdges = entryInfo.out_edges;
                for ( const Edge& edge : outEdges )
                {
                    if ( edge.second == node_name )
                    {
                        if ( edge_values.has_key( edge ))
                        {
                            edge_values.erase(edge);
                        }
                        entryInfo.out_edges.erase( edge );
                    }

                }
                EdgeSet inEdges = entryInfo.in_edges;
                for ( const Edge& edge : inEdges )
                {
                    if ( edge.first == node_name )
                    {
                        if ( edge_values.has_key( edge ))
                        {
                            edge_values.erase(edge);
                        }
                        entryInfo.in_edges.erase( edge );
                    }
                }
            }
        }
...
//Hint: Simpler than remove_node: write and test this one first
    ...
    {
        Edge theEdge = Edge( origin, destination );
        if ( edge_values.has_key( theEdge ))
        {
            edge_values.erase( theEdge );
            for ( NodeMapEntry& mapEntry : node_values )
            {
                LocalInfo& entryInfo = mapEntry.second;
                entryInfo.out_edges.erase( theEdge );
                entryInfo.in_edges.erase( theEdge );
                if ( mapEntry.first == origin )
                {
                    entryInfo.out_nodes.erase( destination );
                }
                if ( mapEntry.first == destination )
                {
                    entryInfo.in_nodes.erase( origin );
                }
            }
        }
    }
...
//Hint: use split and istringstream (the extraction dual of ostreamstring)
    ...
    void HashGraph<T>::load( std::ifstream& in_file, std::string separator )
    {
        ...
        while ( in_file >> line )
        {
            auto tokens = ics::split( line, separator );
            if ( tokens.size() > 1 )
            {
                std::istringstream strStream( tokens[ 2 ] );
                T edgeValue;
                strStream >> edgeValue;
                this->add_edge( tokens[ 0 ], tokens[ 1 ], edgeValue );
            }
            else{
                this->add_node( tokens[ 0 ] );
            }
        }
    }
...
//Hint: this is the easier of the two methods: write and test it first
    ...
    {
        for ( const auto& nodeMapEntry : node_values )
        {
            out_file << nodeMapEntry.first << std::endl;
        }
        for ( const auto& edgeMapEntry : edge_values )
        {
            const auto& edge = edgeMapEntry.first;
            out_file << edge.first << separator << edge.second << separator << edgeMapEntry.second << std::endl;
        }
    }
...
//Hint: each copied LocalInfo object should reset from_graph to the this new graph
    ...
    HashGraph<T>& HashGraph<T>::operator=( const HashGraph<T>& rhs )
    {
        ...
        this->clear();
        for ( const NodeMapEntry& nodeMapEntry : rhs.node_values )
        {
            node_values[ nodeMapEntry.first ] = nodeMapEntry.second;
            node_values[ nodeMapEntry.first ].from_graph = this;
        }
...
//  just check that node names are the same in each
    ...
    bool HashGraph<T>::operator==( const HashGraph<T>& rhs ) const
    {
        ...
        return this->node_values == rhs.all_nodes();
    }
...
//Return whether two graphs are different
    ...
    std::ostream& operator<<( std::ostream& outs, const HashGraph<T>& g )
    {
        ...
        outs << "graph[" << std::endl;
        for ( const auto& nodeMapEntry: nodes )
        {
            outs << nodeMapEntry.first << " -> " << nodeMapEntry.second << std::endl;
        }
