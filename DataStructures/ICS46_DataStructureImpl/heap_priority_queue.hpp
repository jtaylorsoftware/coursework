...
//The (unique) non-undefinedgt value supplied by tgt/cgt is stored in the instance variable gt.
    ...
    private:
        ...
        T* pq;                              //Array stores a heap, organized by the heap ordering property
...
//Destructor/Constructors
            ...
            : gt( tgt != ( gtfunc ) undefinedgt<T> ? tgt : cgt ), length( i.size())
    {
        ...
        used = length;
        int pq_i = 0;
        for ( auto it = i.begin(); it != i.end(); ++it, ++pq_i )
        {
            pq[ pq_i ] = *it;
        }
        heapify();
...
//Operators
    ...
    bool HeapPriorityQueue<T, tgt>::operator==( const HeapPriorityQueue<T, tgt>& rhs ) const
    {
        ...
        for ( auto lhs_i = begin(); lhs_i != lhs_end; ++lhs_i, ++rhs_i )
            // Use iterators for both in order to access in order of priority.
            // This is necessary because the insertion order may cause a different order in the tree
            // but the trees are still equal.
            if ( !( *lhs_i == *rhs_i ))
                return false;
...
//Private helper methods
    ...
    {
        if ( in_heap( left_child( i )) && !gt( pq[ i ], pq[ left_child( i ) ] ))
        {
            return false;
        }
        if ( in_heap( right_child( i )) && !gt( pq[ i ], pq[ right_child( i ) ] ))
        {
            return false;
        }
        return true;
    ...
    void HeapPriorityQueue<T, tgt>::percolate_down( int i )
    {
        ...
        for ( ; in_heap( i ); )
        {
            int higherPrioChild = i;
            if ( in_heap( left_child( i )) && in_heap( right_child( i )))
            {
                higherPrioChild = gt( pq[ left_child( i ) ], pq[ right_child( i ) ] ) ? left_child( i ) : right_child( i );
            }
            else if ( in_heap( left_child( i )))
            {
                higherPrioChild = left_child( i );
            }
            else if ( in_heap( right_child( i )))
            {
                higherPrioChild = right_child( i );
            }

            if ( gt( pq[ higherPrioChild ], pq[ i ] ))
            {
                std::swap( pq[ higherPrioChild ], pq[ i ] );
                i = higherPrioChild;
            }
            else
            {
...
//Iterator class definitions
    ...
    T HeapPriorityQueue<T, tgt>::Iterator::erase()
    {
        ...
        T toReturn = it.dequeue();

        int i = 0;
        for ( ; i < ref_pq->used; ++i )
        {
            if ( ref_pq->pq[ i ] == toReturn )
                break;
        }

        ref_pq->pq[ i ] = ref_pq->pq[ --ref_pq->used ];
        if ( ref_pq->gt( ref_pq->pq[ ref_pq->parent( i ) ], ref_pq->pq[ i ] ))
        {
    ...
    {
        std::ostringstream answer;
        answer << ref_pq->str() << "/current=" << ( !it.empty() ? it.peek() : "" ) << "/expected_mod_count=" << expected_mod_count << "/can_erase=" << can_erase;
        return answer.str();
    }
    ...
    bool HeapPriorityQueue<T, tgt>::Iterator::operator==( const HeapPriorityQueue<T, tgt>::Iterator& rhs ) const
    {
        ...
        return ref_pq == rhs.ref_pq && it.used == rhs.it.used;
    }
