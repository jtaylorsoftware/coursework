/** 
 * unrm: unremoves files that were removed by srm (safe-remove), restoring them to the
 * current directory
 * usage: $ ./unrm [files or directories that exist in trash]
 * 
 * notes: the TRASH environment variable must be set to the path of an existing directory
 * in order to run unrm; otherwise, an error is printed and srm will quit. unrm will 
 * overwrite files that share a name.
 * 
 */
...
const char* ERROR_NO_TRASH_ENV = "urm: error: TRASH environment variable must be set to path of a directory\n";
const char* WARN_NO_FILE = "urm: unable to unremove \"%s\": No such file or directory \n";
const char* WARN_UNRM_ROOT = "urm: cannot unremove on root, \"/\"\n";
const char* ERROR_CANT_CREATE = "urm: error: unable to unremove \"%s\" (could not create a destination file)\n";
...
int main(int argc, char* argv[]){
    ...
    // move files from trash to current directory
    int i;
    for (i = 1; i < argc; ++i){
        char* dstPath = ".";
        char* srcPath = createPath(trashDirPath, argv[i]);
        moveContents(dstPath,srcPath); // unrm is basically the same as srm, except here src is from TRASH
        free(srcPath);
    }
